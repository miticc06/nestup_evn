...
3. Sensors về **tiền điện ngày / tháng** chỉ mang tính chất tham khảo, được tính một cách **thủ công** theo [giá bán lẻ bên dưới](https://github.com/miticc06/nestup_evnnew#2-gi%C3%A1-b%C3%A1n-l%E1%BA%BB-%C4%91i%E1%BB%87n-evn), vì vậy khả năng **sai số** là **rất cao**.
4. Các thông tin không được cập nhật **tức thì** từ dữ liệu mới nhất của EVN, mà luôn được cập nhật theo [chu kì cố định](https://github.com/miticc06/nestup_evnnew#3-chu-k%C3%AC-c%E1%BA%ADp-nh%E1%BA%ADt-d%E1%BB%AF-li%E1%BB%87u-m%E1%BB%9Bi-t%E1%BB%AB-evn).
...
5. **Ngày** cập nhật dữ liệu mới nhất cùng với ngày đầu kì.
    ...
    > Xem thêm [Ý nghĩa của các sensors](https://github.com/miticc06/nestup_evnnew#1-%C3%BD-ngh%C4%A9a-c%E1%BB%A7a-c%C3%A1c-sensor-%C4%91%C6%B0%E1%BB%A3c-t%E1%BA%A1o-s%E1%BA%B5n) phía dưới để hiểu rõ chức năng / hạn chế của từng thông số trên.
...
- Không phải tất cả công tơ **điện tử đo xa** đều hỗ trợ **ghi theo ngày**.
    ...
    > Để đảm bảo công tơ nhà bạn đủ điều kiện để sử dụng công cụ, xin hãy truy cập vào link [Đăng nhập](https://github.com/miticc06/nestup_evnnew#3-m%C3%A3-kh%C3%A1ch-h%C3%A0ng-v%C3%A0-t%C3%A0i-kho%E1%BA%A3n-evn) phía dưới (xin chọn tương ứng với khu vực EVN của bạn).
...
> HACS > Integrations > ➕ Explore & download repositories  > `EVN New` > `Chọn phiên bản mới nhất` > Download this repository
    
3. Khởi động lại HomeAssistant.
4. Cài đặt thành công, tiếp theo đến bước [Config](https://github.com/miticc06/nestup_evnnew#thi%E1%BA%BFt-l%E1%BA%ADp-v%C3%A0-ch%E1%BB%89nh-s%E1%BB%ADa) để hoàn thành.

### Cách 2: Cài đặt thủ công thông qua Samba / SFTP
1. Tải phiên bản mới nhất của công cụ thông qua [Github](https://github.com/miticc06/nestup_evnnew.git).

2. Giải nén thư mục `custom_components/nestup_evnnew` vào thư mục `custom_components` trong HomeAssistant của bạn.
...
> Không có những người kể trên, repo `nestup_evnnew` sẽ không ở đây, trở thành một dự án open-source tới cộng đồng HA Việt Nam.
